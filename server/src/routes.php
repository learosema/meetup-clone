...
$app->get('/env', function (Request $request, Response $response, $args) {
  if ($request->getUri()->getHost() !== 'localhost') {
    return $response
      ->withStatus(403)
      ->withJson(['response' => 'Permission denied.']);
  }
...
$app
  ->get('/auth', function (Request $request, Response $response) {
...
// Get User By ID
$app
  ->get('/user/{id}', function (Request $request, Response $response, $args) {
...
$app->post('/user/{id}', function (
  ...
  if ($this->userService->getUserById($user['id'])) {
    return $response
      ->withStatus(409)
      ->withJson(['response' => 'User already exists.']);
  }
...
$app
  ->put('/user/{id}', function (Request $request, Response $response, $args) {
    $user = $request->getParsedBody();
    $userId = $args['id'];
    $user['id'] = $userId;
...
$app
  ->delete('/user/{id}', function (
    ...
    if ($this->userService->deleteUser($userId)) {
      ...
      return $response
        ->withStatus(404)
        ->withJson(['response' => 'User not found.']);
    }
  })
  ->add($auth);
...
// Create group
$app
  ->post('/group/{id}', function (Request $request, Response $response, $args) {
    try {
...
// Update group
$app
  ->put('/group/{id}', function (Request $request, Response $response, $args) {
    $groupId = $args['id'];
    if ($this->identity->role === "admin") {
      $userRole = "admin";
    } else {
      $userRole = $this->groupService->getGroupMemberRole(
        $groupId,
        $this->identity->id
      );
    }
    if ($userRole === false) {
      return $response
        ->withStatus(403)
        ->withJson(['response' => 'Not in group or group does not exist.']);
    }
    if ($userRole !== "admin") {
      return $response->withStatus(403)->withJson([
        'response' => 'Insufficient permissions to delete the group.'
      ]);
    }
    $group = $request->getParsedBody();
    $group['id'] = $groupId;
    if ($this->groupService->updateGroup($group)) {
      ...
      return $response
        ->withStatus(404)
        ->withJson(['response' => 'Group not found.']);
    }
  })
  ->add($auth);
...
// Get group members
$app->get('/group/{id}/members', function (
  Request $request,
  Response $response,
  $args
) {
...
// Add user to group
$app
  ->post('/group/{id}/members/{mid}', function (
    Request $request,
    Response $response,
    $args
  ) {
    $groupId = $args['id'];
    $userId = array_key_exists('mid', $args)
      ? $args['mid']
      : $this->identity->id;
    $group = $this->groupService->getGroupById($groupId);
    if (!$group) {
    ...
    if ($this->groupService->addMember($groupId, $userId)) {
      return $response->withJson([
        'response' => "User $userId added to group $groupId."
      ]);
    } else {
      return $response
        ->withStatus(409)
        ->withJson(['response' => "User $userId already in group $groupId."]);
    }
  })
  ->add($auth);
...
$app
  ->delete('/group/{id}/members/{mid}', function (
    Request $request,
    Response $response,
    $args
  ) {
    ...
    if ($this->groupService->deleteMember($groupId, $userId)) {
      return $response->withJson([
        'response' => "User $userId deleted from group $groupId."
      ]);
    } else {
      return $response
        ->withStatus(404)
        ->withJson(['response' => "User $userId not in group $groupId."]);
    }
  })
  ->add($auth);
...
$app->get('/group/{id}/events', function (
  Request $request,
  Response $response,
  $args
) {
...
// Create new event
$app
  ->post('/group/{id}/event/{eid}', function (
    Request $request,
    Response $response,
    $args
  ) {
    ...
    if ($event) {
      return $response
        ->withStatus(409)
        ->withJson(['response' => 'Event already exists. Use PUT to update.']);
    }
...
// Update event
$app
  ->put('/group/{id}/event/{eid}', function (
    Request $request,
    Response $response,
    $args
  ) {
    ...
    if (!$event) {
      return $response
        ->withStatus(404)
        ->withJson(['response' => 'Event not found.']);
    }
    $event = $request->getParsedBody();
    $event['id'] = $eventId;
    $event['group_id'] = $groupId;
...
// Delete event
$app
  ->delete('/group/{id}/event/{eid}', function (
    Request $request,
    Response $response,
    $args
  ) {
...
// Submit RSVP
$app
  ->post('/group/{id}/event/{eid}/rsvp', function (
    Request $request,
    Response $response,
    $args
  ) {
    ...
    if (
      !array_key_exists('rsvp', $data) ||
      $data['rsvp'] !== 'yes' ||
      $data['rsvp'] !== 'no'
    ) {
      return $response
        ->withStatus(422)
        ->withJson(['response' => 'Parameter rsvp missing.']);
    }
    if ($this->identity->role === "admin") {
      $userRole = "admin";
    } else {
