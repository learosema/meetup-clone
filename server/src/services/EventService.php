...
class EventService
{
  ...
  public function getGroupEvents($groupId)
  {
    $query = $this->db->prepare(
      'SELECT * FROM group_events WHERE group_id = :group_id'
    );
  ...
  {
    $query = $this->db->prepare(
      'SELECT * FROM group_events WHERE group_id = :group_id AND id = :event_id'
    );
    $query->execute([
      ':group_id' => $groupId,
      ':event_id' => $eventId
  ...
  public function addGroupEvent($event)
  {
    ...
    $vals = array_map(function ($col) {
      return ':' . $col;
    }, $cols);
    $strCols = implode(', ', $cols);
    $strVals = implode(', ', $vals);
    $query = $this->db->prepare(
      "INSERT INTO group_events ($strCols) VALUES ($strVals)"
    );
    $query->execute([
      ':id' => $event['id'],
  ...
  {
    $query = $this->db->prepare(
      'DELETE FROM group_events WHERE id = :id AND group_id = :group_id'
    );
    $query->execute([
      ':id' => $eventId,
      ':group_id' => $groupId
  ...
  public function updateGroupEvent($event)
  {
    ...
    $query = $this->db->prepare(
      "UPDATE group_events SET $strUpdates WHERE id = :id"
    );
    $query->execute([
      ':id' => $event['id'],
      ':group_id' => $event['group_id'],
  ...
  {
    $query = $this->db->prepare(
      'SELECT * FROM rsvp WHERE group_id = :group_id AND event_id = :event_id'
    );
    $query->execute([
      ':group_id' => $groupId,
      ':event_id' => $eventId
  ...
  {
    $query = $this->db->prepare(
      'SELECT COUNT(*) as count FROM rsvp WHERE group_id = :group_id AND event_id = :event_id AND user_id = :user_id'
    );
    $query->execute([
      ':group_id' => $groupId,
      ':event_id' => $eventId,
  ...
  {
    $query = $this->db->prepare(
      "UPDATE rsvp SET rsvp = :rsvp, timestamp = :timestamp WHERE group_id = :group_id AND event_id = :event_id AND user_id = :user_id"
    );
    $query->execute([
      ':group_id' => $groupId,
      ':event_id' => $eventId,
